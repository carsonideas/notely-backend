...
const getNote = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const note = await prisma.entry.findUnique({
      where: { id: noteId },
      include: {
        author: {
          select: {
...
const updateNote = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const existingNote = await prisma.entry.findUnique({
      where: { id: noteId },
    });
...
const getDeletedNotes = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { user } = req.body;
    const userId = user?.id;
    ...
    const deletedNotes = await prisma.entry.findMany({
      where: {
        isDeleted: true,
        authorId: userId,
      },
...
const restoreNote = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const restoredNote = await prisma.entry.update({
      where: { id: noteId },
      data: {
        isDeleted: false,
        updatedAt: new Date(),
